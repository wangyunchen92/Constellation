...
- (void)viewDidLoad {
    ...
    self.title = NSLocalizedString(@"key_detect_result", nil);
...
- (void)buildView {
    UIButton* detectButton = [[UIButton new] faceIDSDKTestButtonTitle:NSLocalizedString(@"title_sure", nil)
...
- (void)setResultData:(FaceIDData *)resultData {
    ...
    [self.messageLabel setText:[NSString stringWithFormat:@"%@ %@", NSLocalizedString(@"key_action_live_detect", nil), self.isDetectFinish ? NSLocalizedString(@"title_success", nil) : NSLocalizedString(@"title_unsuccess", nil)]];
}
