...
- (void)viewDidLoad {
    ...
    LHSIDCardScaningView *IDCardScaningView = [[LHSIDCardScaningView alloc] initWithFrame:self.view.frame isFront:self.isFront];
    self.faceDetectionFrame = IDCardScaningView.facePathRect;
    // 设置 扫描view
    DDLog(@"isfront == %d",self.isFront);
    IDCardScaningView.isFront = self.isFront;
...
//            iDInfo.type = ctype;            
            ...
            if (iDInfo) {// 读取到身份证信息，实例化出IDInfo对象后，截取身份证的有效区域，获取到图像
                ...
                dispatch_async(dispatch_get_main_queue(), ^{
                    weakSelf.idInfoBlock(iDInfo, subImage, weakSelf.isFront);
                    [weakSelf.navigationController popViewControllerAnimated:YES];
                });
            }
        }
