...
- (instancetype)initWithFrame:(CGRect)frame isFront:(BOOL)isFront{
    if (self = [super initWithFrame:frame]) {
        ...
        // 添加扫描窗口
        [self addScaningWindowWithIsFront:isFront];
...
#pragma mark - 添加扫描窗口
-(void)addScaningWindowWithIsFront:(BOOL)isFront {
    // 中间包裹线
    ...
    if (isFront == YES) {
        ...
        [self addTipLabelWithText:@"将国徽面置于此区域内，国徽对准，扫描" center:center];
        CGFloat facePathWidth  = iPhone5or5cor5sorSE ? 70 : (iPhone6or6sor7 ? 80 : 90);
        CGFloat facePathHeight = facePathWidth * 0.89;
        CGRect rect = _IDCardScanningWindowLayer.frame;
        
        DDLog(@"rect  x == %f y == %f width == %f height == %f",rect.origin.x,rect.origin.y,rect.size.width,rect.size.height);
      //  CGRect newRect = [_IDCardScanningWindowLayer convertRect:rect toLayer:self.window.layer];
      //  [ convertRect:_IDCardScanningWindowLayer.frame toViewOrWindow:self.window];
        self.facePathRect = (CGRect){ CGRectGetMaxX(rect) - facePathWidth - 35 ,CGRectGetMinY(rect) + facePathHeight - 20,facePathWidth,facePathHeight };//+ 10
        // 人像
        UIImageView *headIV = [[UIImageView alloc] initWithFrame:_facePathRect];
        headIV.image = [UIImage imageNamed:@"icon_img_2"];
//        headIV.transform = CGAffineTransformMakeRotation(M_PI * 0.5);
