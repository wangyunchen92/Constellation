...
- (void)setUpPreviewLayerWithType:(FMVideoViewType )type
{
    ...
    switch (type) {
        ...
        case TypeFullScreen:
            ...
            break;
        case TypeCustom:
            rect = CGRectMake(0, 0, SCREEN_WIDTH , SCREEN_WIDTH * 1.25);
            break;
        default:
            rect = [UIScreen mainScreen].bounds;
            break;
...
- (void)reset
{
    ...
    [self.session startRunning];
     //自动聚焦一次
     [self focusWithMode:AVCaptureFocusModeAutoFocus exposureMode:AVCaptureExposureModeAutoExpose atPoint:self.superView.center];
}
