...
#pragma mark - 获取本地图片
+(UIImage* )getLocalFileWithFilePath:(NSString* )filePath  fileKey:(NSString* )fileKey{
    NSMutableData *dataR = [[NSMutableData alloc] initWithContentsOfFile:filePath];
    NSKeyedUnarchiver *unarchiver = [[NSKeyedUnarchiver alloc] initForReadingWithData:dataR];
    UIImage* image = [unarchiver decodeObjectForKey:fileKey];
    [unarchiver finishDecoding];
    return image;
}

//获取本地二进制文件
+(NSData* )getDataFileWithFilePath:(NSString* )filePath  fileKey:(NSString* )fileKey{
    NSMutableData *dataR = [[NSMutableData alloc] initWithContentsOfFile:filePath];
    NSKeyedUnarchiver *unarchiver = [[NSKeyedUnarchiver alloc] initForReadingWithData:dataR];
    NSData* data = [unarchiver decodeObjectForKey:fileKey];
    [unarchiver finishDecoding];
    return data;
}

//本地存储 文件 图片
+(void)writeLoacalFileWithFileData:(id)fileData dataKey:(NSString* )dataKey filePath:(NSString* )filePath{
    //存储正面身份证
    NSMutableData *data = [[NSMutableData alloc] init];
    NSKeyedArchiver *archvier = [[NSKeyedArchiver alloc] initForWritingWithMutableData:data];
    //对多个对象进行归档
    [archvier encodeObject:fileData forKey:dataKey];
