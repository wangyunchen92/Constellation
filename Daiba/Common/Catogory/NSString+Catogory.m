...
+(NSString* )deleteStrWithOrignalStr:(NSString* )orignalStr  specailStr:(NSString* )specailStr{
    DDLog(@"orignalStr == %@  specialStr == %@",orignalStr,specailStr);
    ...
    if ([mutStr containsString:specailStr]) {
        ...
        DDLog(@"newMutStr == %@  strArr == %@",newMutStr,strArr);
        return newMutStr;
    }
    return mutStr;
}
...
+(NSString* )addStrWithOrgnalStr:(NSString* )orgnalStr range:(NSRange)range specalStr:(NSString* )specalStr{
    DDLog(@"orignalStr == %@  specialStr == %@",orgnalStr,specalStr);
...
+(NSString* )statusByUserModel:(UserModel *)usrModel{
    if ((usrModel.status.integerValue == 0  || usrModel.status.integerValue == 1 )&& usrModel.postion.integerValue == 99) {
        return  @"借款处理中...";
    }
    if (usrModel.status.integerValue == 2 && usrModel.postion.integerValue == 99) {
        return  @"借款已到账";
    }
    if (usrModel.status.integerValue == 3 && usrModel.postion.integerValue == 99) {
        return  @"借款失败";
    }
    //开始还款
    if (usrModel.status.integerValue == 4 && usrModel.postion.integerValue == 99) {
        return  @"当前应还金额";
    }
    //还款处理 申请
    if (usrModel.status.integerValue == 5 && usrModel.postion.integerValue == 99) {
        return  @"还款处理中...";
    }
    //还款成功
    if (usrModel.status.integerValue == 6 && usrModel.postion.integerValue == 99) {
        return  @"还款成功";
    }
    //还款失败
    if (usrModel.status.integerValue == 7 && usrModel.postion.integerValue == 99) {
        return  @"还款失败，请重新还款";
    }
    return @"";
}
...
+ (NSString*)dictionaryToJson:(NSDictionary *)dic
{
    ...
    NSData *jsonData = [NSJSONSerialization dataWithJSONObject:dic options:nil error:&parseError];//NSJSONWritingPrettyPrinted
