...
- (instancetype)initWithFrame:(CGRect)frame Titles:(nonnull NSArray *)titles
{
    if ([super initWithFrame:frame])
    {
        ...
        _lineUndo.backgroundColor = unSelectColor;
        ...
        for (NSString *title in _titles)
        {
            ...
            cricle.backgroundColor = unSelectColor;
...
- (void)setStepIndex:(int)stepIndex
{
    if (stepIndex >= 0 && stepIndex < self.titles.count)
    {
        ...
        if (_isFailure == YES) {
            _lblIndicator.text = @"×";
           // _lblIndicator.font = Font(14);
           // _lblIndicator.backgroundColor = FailureCOLOR;
            _lblIndicator.textColor = FailureCOLOR;
            _lineUndo.backgroundColor = FailureCOLOR;
            _lblIndicator.layer.borderColor = [FailureCOLOR CGColor];
//            UIView* circlView =  ((UIView *)[_cricleMarks objectAtIndex:_stepIndex]);
//            circlView.backgroundColor = FailureCOLOR;
           // _lineDone.backgroundColor = FailureCOLOR;
        }
        ...
        for (int i = 0; i < _titles.count; i++)
        {
            ...
            if (cricle != nil)
            {
                if (i <= _stepIndex)
                {
                    ...
                    if (i == stepIndex && _isFailure == YES) {
                        cricle.backgroundColor = FailureCOLOR;
                    }
                }
                else
                {
            ...
            if (lbl != nil)
            {
                if (i <= stepIndex)
                {
                    ...
                    if (i == stepIndex && _isFailure == YES) {
                        lbl.textColor = FailureCOLOR;
                    }
                }
                else
                {
                    lbl.textColor = TitleTextColor;//[UIColor lightGrayColor];
                }
            }
        }
    }
