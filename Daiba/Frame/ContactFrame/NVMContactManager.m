...
- (NSString *)description {
    ...
    return [NSString stringWithFormat:@"%@%@%@--%@",self.lastName ?: @"",self.middleName ?: @"",self.firstName ?: @"",[phoneNumbers copy]];
    //[super description],
}
-(NSString *)fullName{
  return  [NSString stringWithFormat:@"%@%@%@",self.lastName ?: @"",self.middleName ?: @"",self.firstName ?: @""];
}
-(NSString *)fullPhoneNumbers{
    NSMutableString *phoneNumbers = [[NSMutableString alloc] init];
    for (NSString *phoneNum in self.phoneNumbers) {
        [phoneNumbers appendString:phoneNum];
        [phoneNumbers appendString:@"\n"];
...
#pragma clang diagnostic ignored "-Wunused-variable"
    ...
    for ( int i = 0; i < numberOfPeople; i++){
       ...
       // NSLog(@"name == %@%@%@",lastName ?: @"",middlename ?: @"",firstName ?: @"");
...
//                //读取middlename
//                NSString *middlname = (NSString*)CFBridgingRelease(ABRecordCopyValue(person, kABPersonMiddleNameProperty));
//                //读取prefix前缀
...
//                //读取电话多值
//                ABMultiValueRef phonesArr = ABRecordCopyValue(person, kABPersonPhoneProperty);
//                for (int k = 0; k<ABMultiValueGetCount(phonesArr); k++)
//                {
//                    //获取电话Label≥
//                    NSString * personPhoneLabel = (NSString*)CFBridgingRelease(ABAddressBookCopyLocalizedLabel(ABMultiValueCopyLabelAtIndex(phonesArr, k)));
//                    //获取該Label下的电话值
//                    NSString * personPhone = (NSString*)CFBridgingRelease(ABMultiValueCopyValueAtIndex(phonesArr, k));
//        
//                }
//                CFRelease(phonesArr);
//        
