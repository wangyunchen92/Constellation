...
///alertView  宽
#define AlertW   0.8 * SCREEN_WIDTH
///各个栏目之间的距离
#define XLSpace 10.0
...
- (instancetype)initWithTitle:(NSString *)title imgStr:(NSString* )imgStr message:(NSString *)message sureBtn:(NSString *)sureTitle cancleBtn:(NSString *)cancleTitle
{
    if (self == [super init]) {
        ...
        if (message) {
            ...
            self.msgLbl = [self GetAdaptiveLable:CGRectMake(XLSpace, CGRectGetMaxY(_imgView.frame)+XLSpace, AlertW - 2 * XLSpace, 20) AndText:message andIsTitle:NO];
            ...
            self.msgLbl.frame = _imgView ? CGRectMake((AlertW-msgW)/2, CGRectGetMaxY(_imgView.frame)+XLSpace, msgW, msgH):CGRectMake((AlertW-msgW)/2, 2*XLSpace, msgW, msgH);
        }
        ...
        if(sureTitle && !cancleTitle){
            ...
            [YMTool viewLayerWithView:_sureBtn cornerRadius:20 boredColor:ClearColor borderWidth:1];
